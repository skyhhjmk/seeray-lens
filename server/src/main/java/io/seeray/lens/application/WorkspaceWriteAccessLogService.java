...
/** Owner/admin view of recent human-user API writes, with route and status metadata only. */
@ApplicationScoped
public class WorkspaceWriteAccessLogService {
    ...
    public WorkspaceWriteAccessLogService(DataSource dataSource, WorkspaceAccess access) {
    ...
    public Page list(UUID workspaceId, String fromValue, String toValue, String cursorValue, int requestedLimit) {
        ...
        String sql = "select l.id,l.actor_user_id,u.email,l.site_id,s.name,l.method,l.route_template,"
                + "l.status_code,l.created_at from workspace_api_write_log l "
                + "left join app_user u on u.id=l.actor_user_id left join site s on s.id=l.site_id "
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            throw new IllegalStateException("Could not read workspace API write history", error);
        }
