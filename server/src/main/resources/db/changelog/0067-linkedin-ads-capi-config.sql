CREATE TABLE analytics_linkedin_ads_capi_config (
...
CREATE TABLE analytics_linkedin_ads_goal_mapping (
  site_id UUID NOT NULL REFERENCES site(id) ON DELETE CASCADE,
  goal_id UUID NOT NULL REFERENCES goal_definition(id) ON DELETE CASCADE,
  conversion_urn VARCHAR(80) NOT NULL,
  updated_by UUID NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  PRIMARY KEY (site_id, goal_id),
  CHECK (conversion_urn ~ '^urn:lla:llaPartnerConversion:[0-9]{1,32}$')
);
