...
class ControlPlaneResourceTest {
    ...
    void persistedAnalyticsEventsEnterExtensionOutboxWithoutVisitorIdentifiers() throws Exception {
        ...
        String deliveryId = given().header("Authorization", "Bearer " + owner.access())
                ...
                .path("[0].id");
        given().header("Authorization", "Bearer " + owner.access())
                .get(path + "/" + extensionId + "/deliveries/summary")
                .then()
                .statusCode(200)
                .body("total", is(1))
                .body("pending", is(1))
    ...
    void clientCrashCollectorRedactsSensitiveDataAndReportClustersAnonymousErrors() throws Exception {
        ...
        String nativeEvent = "{\"eventId\":\"" + UUID.randomUUID() + "\",\"type\":\"client_error\","
                + "\"occurredAt\":\"" + occurred.plusSeconds(2) + "\",\"url\":\"https://example.com/checkout\","
                + "\"category\":\"error\",\"action\":\"native_android\",\"name\":\"IllegalStateException\","
                + "\"properties\":{\"errorName\":\"IllegalStateException\","
        ...
        try (var connection = dataSource.getConnection();
                var statement = connection.prepareStatement(
                        "select page_path,client_visitor_id,client_session_id,event_data::text "
                                ...
                                + "and event_data->'data'->>'platform'='android'")) {
            statement.setObject(1, UUID.fromString(site));
            try (var result = statement.executeQuery()) {
                assertTrue(result.next());
                assertEquals("/checkout", result.getString(1));
                ...
                assertTrue(stored.contains("IllegalStateException"));
                assertTrue(stored.contains("\"platform\": \"android\""));
                assertTrue(stored.contains("/assets/app.js"));
                assertFalse(stored.contains("native-secret"));
                assertFalse(stored.contains("bob@example.test"));
                assertFalse(result.next());
            }
        }
        try (var connection = dataSource.getConnection();
