...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.scrollUntilVisible(
        find.text('Send conversions to Meta Ads'),
        350,
        scrollable: find.byType(Scrollable).first,
      );
  ...
  testWidgets(
    'renders Meta destination and persists the selected goal mapping',
    (tester) async {
      final api = _OfflineConversionsApi(metaAdsConfigured: true);
      await tester.pumpWidget(
        ProviderScope(
          overrides: [apiProvider.overrideWithValue(api)],
          child: MaterialApp(
      ...
      expect(find.text('987654321 · USD'), findsOneWidget);
      expect(find.text('Qualified lead → Lead'), findsOneWidget);
      expect(find.text('Preview imported Meta Ads CSV'), findsOneWidget);
      await tester.tap(find.text('Save mapping'));
      await tester.pumpAndSettle();
      expect(api.requests, hasLength(1));
      expect(
        api.requests.single['path'],
        '/api/v1/sites/site-1/offline-conversions/meta-ads/config/goals/goal-1',
      );
      expect(api.requests.single['body'], {'eventName': 'Lead'});
      expect(tester.takeException(), isNull);
    },
  );
...
class _OfflineConversionsApi extends SeeRayApi {
  ...
  Future<dynamic> request(
    ...
    if (uri.path.contains('/offline-conversions/meta-ads/config/goals/')) {
      requests.add({'method': method, 'path': path, 'body': body});
  ...
  Map<String, dynamic> get _metaAdsConfig => {
    'canManage': true,
    'configured': metaAdsConfigured,
    'credentialConfigured': metaAdsConfigured,
    'datasetId': metaAdsConfigured ? '987654321' : null,
    'currencyCode': metaAdsConfigured ? 'USD' : null,
    'goalMappings': metaAdsConfigured
        ? [
            {
              'goalId': 'goal-1',
              'goalName': 'Qualified lead',
