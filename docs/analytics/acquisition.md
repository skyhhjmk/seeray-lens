...
Attribution is a reporting model, not causal evidence. Web conversions and their touches are joined through the site's pseudonymous visitor/session facts; clearing visitor storage or switching devices can fragment a journey. The Acquisition → Offline conversions page separately accepts a normalized UTF-8 CSV with `conversion_id,platform,click_id,converted_at`. A file is assigned to one enabled goal, limited to 2 MiB and 5,000 rows, validated before import, and idempotent by normalized content. Conversion IDs and click IDs are site-scoped hashed before storage; raw identifiers and filenames are not retained. Reusing a conversion ID in another file is rejected. Offline rows match the latest same-platform click-ID session at or before conversion time within the selected 7-, 30-, or 90-day window; multi-touch credit then uses the eligible visitor's sessions. Goal fixed value is used for attributed value; ecommerce revenue and currency are not imported. A selected saved segment filters the matched click session while prior eligible touches remain in the lookback window. Imported conversion rows follow site aggregate retention. Attribution remains modeled credit, not causal evidence; external ad-platform export and browser acceptance remain outstanding. The Acquisition page links to graphical model controls; no JSON configuration is exposed to operators.
