...
- Custom report: group by event type, registered custom dimensions, discovered nested event-property paths, or session dimensions such as entry/exit page or title, referrer, campaign fields, visitor type, browser/OS/device, language, and available location fields. Choose up to four distinct dimensions for a cross-breakdown. The searchable property browser lists scalar paths observed in the selected date range and saved segment (up to 250 paths found in the newest 10,000 matching events, nested at most five levels); it reports path and event/visit counts, never example values. Common credential and contact fields are excluded. When event type or an event property participates, event-level dimensions are joined from the same raw event; an event without that property is grouped under “Unknown”. Measures include visits, unique visitors, page views, events, bounced visits, average duration, bounce rate, and a named calculated metric. Calculated metrics combine two allowlisted measures with addition, subtraction, multiplication, or division, and can be displayed as a number or percentage; division by zero returns 0. Choose a table or bar display and add up to five visual audience conditions; conditions can match all or any.
...
Custom reports use server-approved session dimensions, event-type breakdowns, registered custom dimensions, discovered nested event-property paths, and measures, plus the existing safe segment-rule vocabulary. Up to four distinct dimensions can be paired; this is a bounded pivot over observed scalar property paths, not arbitrary JSON access. The path browser samples up to 10,000 matching events and returns at most 250 property paths, without exposing sample values; path values are queried only after a member selects the dimension. Common credential/contact paths are excluded. When a report includes event type or an event property, event-level values are joined from the same raw event; an event without that property is grouped under `Unknown`. Event counts are exact for each selected combination. Visit and visitor measures count a visit/visitor once per combination; visits and unique visitors can recur across rows, so row totals are not additive. Standalone custom-dimension reports group visit-level measures once per distinct value seen during a visit, and calculated-metric event operands count only events carrying that value. Registered custom dimensions follow the separate value semantics documented in [custom dimensions](custom-dimensions.md). Browser acceptance is still pending. Scheduled weekly/monthly email reports cover the overview, top pages, and acquisition sources; comparative alerts are managed from the Alerts tab. See [scheduled email reports](scheduled-reports.md) for timing, permissions, SMTP setup, and API details, and [analytics alerts](alerts.md) for comparative thresholds and delivery setup.
...
GET    /api/v1/sites/{siteId}/analytics/custom-report/event-properties?from=YYYY-MM-DD&to=YYYY-MM-DD
```
