...
The existing-account add flow grants access immediately. Email invitations are a separate opt-in flow: the invitee must accept the link, and the accepted account email must match the invited address. Workspace login/token history, read-access auditing, SSO, and site-scoped grants remain separate work.
