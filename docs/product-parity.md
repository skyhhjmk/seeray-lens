...
| Experimentation | Site-scoped definitions, stable client assignment, URL path-prefix, device, and server-evaluated saved-segment targeting with explicit lookback, exposure events, unique-session conversion reports, control-relative lift/p-value/significance, 95% Wilson/Newcombe-Wilson confidence intervals, visual admin editor, and an adjustable sample-size planner | Experiment lifecycle/mutual-exclusion governance and browser acceptance |
